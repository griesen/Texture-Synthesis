...
im = Image.fromarray((np.tile(x['all_cgrams'][:, :, -42], (3, 1, 1))*255).astype(np.uint8).transpose((1, 2, 0))).resize((225, 225), resample=Image.ANTIALIAS)
im = np.asarray(im).transpose((2, 0, 1)).reshape((1, 3, 225, 225))
print(im.shape)
r, targets = thing4.main(pf, sf, '/home/ubuntu/test_audio_stim2a/test_real.npy', 
                         [(im, [('conv1', 1, 'corr_rs'), 
                                ('pool1', 1, 'corr_rs'), 
                                ('conv2', 1, 'corr_rs'), 
                                ('pool2', 1, 'corr_rs'), 
                                ('conv3', 1000, 'corr_rs'), 
                                ('conv4', 100000, 'corr_rs'),  
                                ('conv5', 100000, 'corr_rs'), 
                                ('pool5', 100000, 'corr_rs')
                            ])], 'data', seed=2, use_bounds=True, 
                         mean=mean, save_dir = '/home/ubuntu/test_audio_stim2a/things/', save_freq=100)
