...
def get_val_and_diff(x, targets, data_layer, start_layer, save_dir, save_freq):
    ...
    for tind, targ in enumerate(target_order[:-1]):
        ...
        for w, t, tp in targets[targ]:
            ...
            val += w * val_d
            diff += w * diff_d
            stats[(targ, tp, 'val')] = val
            stats[(targ, tp, 'diff')] = np.abs(diff).mean()
    ...
    if save_dir:
        if COUNT % save_freq == 0:
            ...
            np.save(diff_file, N.blobs[data_laya].diff)
...
def main(param_file, state_file, save_file, targets0, data_layer='data', crop=None, mean=None, seed=0, use_bounds=False, start_normal=True, start_layer=None, start_im=None, save_dir=None, save_freq=None):
