...
im = Image.fromarray((np.tile(x['all_cgrams'][:, :, -14], (3, 1, 1))*255).astype(np.uint8).transpose((1, 2, 0))).resize((225, 225))
im = np.asarray(im).transpose((2, 0, 1)).reshape((1, 3, 225, 225))
print(im.shape)
r, targets = thing4.main(pf, sf, '/home/ubuntu/test_audio_real2/test_real.npy', 
                         [(im, [#('conv1', 1, 'corr_t'), 
                                #('pool1', 1, 'corr_t'), 
                                #('conv2', 1, 'corr_t'), 
                                #('pool2', 1, 'corr_t'), 
                         ...
                         mean=mean, save_dir = '/home/ubuntu/test_audio_real2/things/', save_freq=100, start_normal='zeros')
